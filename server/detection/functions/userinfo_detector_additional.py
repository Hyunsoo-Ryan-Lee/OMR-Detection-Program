...
def personal_info_additional(
    ...
    multi_, empty_, empty_tot, p_multi, p_empty, fatal_list = [[] for _ in range(6)]
    name_keys = [
        name
            ...
            if len(instance) == 5 or -2 in instance or -3 in instance:
                ...
                for x in xx:
                    ...
                    if sum(i > THRESH for i in boxes) == 0:
                        ...
                        if (
                            "개인번호" in inst_key
                            or "학년/반/번호" in inst_key
                            or "학번" in inst_key
                        ):
                            answer.append("*")
                            fatal_list.append("error")
                            [
                                multi_.append([inst_key, box[i]])
                                for i in range(len(boxes))
                                if boxes[i] > THRESH
                ...
                if sum(i > THRESH for i in boxes) == 0:
                    ...
                    if "개인번호" in inst_key or "학년/반/번호" in inst_key or "학번" in inst_key:
                        answer.append("*")
                        fatal_list.append("error")
                        [
                            multi_.append([inst_key, box[i]])
    ...
    return personal_final_list, personal_error_list, total_order, list(set(fatal_list))
